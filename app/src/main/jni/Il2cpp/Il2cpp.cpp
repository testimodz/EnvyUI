...
namespace Il2cpp
{
    ...
    uint32_t GCHandle_New(Il2CppObject *object, bool pinned)
    {
        return il2cpp_gchandle_new(object, pinned);
    }
    ...
    Il2CppObject* GCHandle_GetTarget(uint32_t gchandle)
    {
        if (gchandle == 0) return nullptr;
        return il2cpp_gchandle_get_target(gchandle);
    }
    
    bool GCHandle_FreeSafe(uint32_t gchandle)
    {
        // Safe version: check if handle is valid before freeing
        // Returns true if freed successfully, false if skipped
        if (gchandle == 0) return false;
        
        // Check if target still exists (handle is valid)
        Il2CppObject* target = il2cpp_gchandle_get_target(gchandle);
        if (target == nullptr) {
            // Handle already invalidated by GC, don't free
            return false;
        }
        
        // Target exists, safe to free
        il2cpp_gchandle_free(gchandle);
        return true;
    }
} // namespace Il2cpp
